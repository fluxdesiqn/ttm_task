 ...
 */
add_filter( 'dynamic_sidebar_params', 'ttm_widget_classes' );

if ( ! function_exists( 'ttm_widget_classes' ) ) {
	 ...
	 * @return array $params
	 */
	function ttm_widget_classes( $params ) {
		...
		if ( isset( $params[0]['id'] ) && strpos( $params[0]['before_widget'], 'dynamic-classes' ) ) {
			...
			if ( 0 === $widget_count % 4 || $widget_count > 6 ) {
				...
				$widget_classes .= ' col-6 col-sm-2 col-md-3';
			} elseif ( 6 === $widget_count ) {
				// If exactly six widgets are published.
				$widget_classes .= ' col-6 col-sm2 col-md-2';
			} elseif ( $widget_count >= 3 ) {
				// Three widgets per row if there's three or more widgets.
				$widget_classes .= ' col-6 col-sm-4';
			} elseif ( 2 === $widget_count ) {
				// If two widgets are published.
				$widget_classes .= ' col-6 col-md-6';
...
} // End of if function_exists( 'ttm_widget_classes' ).

add_action( 'widgets_init', 'ttm_widgets_init' );

if ( ! function_exists( 'ttm_widgets_init' ) ) {
	/**
	 * Initializes themes widgets.
	 */
	function ttm_widgets_init() {
		register_sidebar(
			array(
		...
		register_sidebar(
			array(
				'name'          => __( 'Footer Full', '_themename' ),
				'id'            => 'footerfull',
				'description'   => __( 'Full sized footer widget with dynamic grid', '_themename' ),
...
} // End of function_exists( 'ttm_widgets_init' ).
